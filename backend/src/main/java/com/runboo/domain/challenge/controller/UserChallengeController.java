 ...
 */
    ...
    public List<UserChallengeDto> getUserChallenges(@AuthenticationPrincipal CustomUserDetails user, @PathVariable String status){
        // 서비스에게 유저 ID를 전달하여 가공된 DTO 리스트를 받습니다.
        List<UserChallengeDto> response = userChallengeService.getUserChallengeListByStatus(user.getUserId(), status);
