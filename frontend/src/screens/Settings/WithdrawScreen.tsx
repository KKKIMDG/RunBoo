...
export default function WithdrawScreen({ navigation }: any) {
  ...
  const isDisabled = loading || (isLocal && password.trim().length === 0);
  ...
  const submitWithdraw = async () => {
    try {
      ...
      Alert.alert("계정 탈퇴 완료", "계정이 정상적으로 탈퇴되었습니다.", [
  ...
  const handleWithdraw = () => {
    Alert.alert(
        ...
        [
          ...
          { text: "탈퇴하기", style: "destructive", onPress: submitWithdraw },
        ]
    );
  };
        ...
        >
          ...
          >
            ...
            <View style={styles.noticeBox}>
              <Ionicons name="warning-outline" style={styles.noticeIcon} />
              <View style={styles.noticeTextWrapper}>
                <Text style={styles.noticeTitle}>계정 탈퇴 안내</Text>
                <Text style={styles.noticeDesc}>
                  탈퇴 시 모든 데이터는 복구할 수 없으며,
                  탈퇴 후 한 달간 동일 이메일로 재가입이 불가합니다.
                </Text>
              </View>
            </View>
            ...
            >
              <Text style={styles.withdrawButtonText}>계정 탈퇴하기</Text>
            </TouchableOpacity>
          </ScrollView>
        </KeyboardAvoidingView>
      </SafeAreaView>
